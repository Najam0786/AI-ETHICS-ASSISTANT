...
The supervisor node (#28) is one layer, not the whole defense. Research into RAG evaluation practice (RAGAS-style faithfulness/relevancy metrics, retrieval-confidence abstention) confirmed the single most valuable addition was a **pre-generation gate**: we can't control what a user asks, but we can control whether we attempt an answer at all when the knowledge base doesn't support one — that decision shouldn't be left entirely to the LLM's judgment.

| # | Decision | Alternatives Considered | Rationale |
|---|---|---|---|
| 29 | Retrieval confidence gate (`retrieve` → `abstain`, threshold 0.9 on Chroma L2 distance) | No gate (rely on system prompt's "I don't know" rule only); a fixed low `k` instead of a score check | Deterministic and free — zero extra LLM calls. Calibrated empirically against this corpus: 4 in-scope test questions scored 0.43–0.75, 4 clearly out-of-scope questions (capital of France, cake recipe, stock price, Japanese law) scored 1.02–1.78, and 6 borderline questions (GDPR, neural networks, US AI regulation, penalties for non-compliance) confirmed 0.9 sits cleanly in the gap without blocking genuinely answerable questions. A naive version applied this gate using only the raw current-turn text, which broke conversational memory — a follow-up like "give an example of it" has no self-contained meaning and always scored poorly. Fixed by prepending the prior assistant turn to the *retrieval query only* (not to the question `generate` sees), which restored correct scoring without an extra LLM call for query rewriting. |
| 30 | MMR retrieval (`fetch_k=20`, `lambda_mult=0.5`) instead of plain top-k similarity | Plain top-k similarity (original); re-ranking with a cross-encoder | Plain similarity search can return several near-duplicate chunks from the same passage, effectively giving the LLM less real coverage than TOP_K=4 implies (this is what caused the earlier chroma_db duplication bug to go unnoticed for a while — duplicate chunks looked like normal retrieval results). MMR trades a small amount of pure relevance for diversity, which is a cheap, one-line, no-extra-latency fix; a cross-encoder re-ranker would need a separate model download and inference cost this project's "zero cost, fully local" constraint doesn't want. |
